...
class ConfigValueMixin:
    ...
    def get(cls, item, default=NO_VALUE):
        ...
        if item.lower() not in valid_items and default is cls.NO_VALUE:
            raise KeyError(
                f"{item} is not a valid {cls.__name__}. \
                Must be one of {valid_items} (case insensitive)"
            )
...
class Turtle(TNavigator):
    """Turtle object that to make embroidery files. Mirrored after the
    official :py:mod:`turtle` library.

    This class has the same API as the builtin ``turtle.Turtle`` class with
    documented changes, for more information see the official documentation of
    the `builtin turtle library <https://docs.python.org/3/library/turtle.html
    #methods-of-rawturtle-turtle-and-corresponding-functions>`_.
    ...
    pattern : turtlethread.stitches.EmbroideryPattern (optional)
        The embroidery pattern to work with. If not supplied, then an empty
        pattern will be created.
    scale : float (optional, default=1)
        Scaling between turtle steps and units in the embroidery file. Below
        are some example scaling.
    ...
    def __init__(
        ...
        scale=1,
        angle_mode="degrees",
        mode=TNavigator.DEFAULT_MODE,
    ):
        # TODO: Flag that can enable/disable changing angle when angle mode
        # is changed
        if pattern is None:
        ...
        # Set up stitch parameters prior to super.__init__ since self.reset()
        # depends on stitch type
    ...
    def angle_mode(self, value):
        ...
        if not isinstance(value, (str, AngleMode)):
            raise TypeError(
                f"Angle mode must be one of 'degrees' or 'radians' \
                (case insensitive), not {value}"
            )
    ...
    def _steps_from_stitch_length(self, stitch_length, radius, extent):
        if radius == 0:
            ...
            steps = 4
            warn(
                "Cannot calculate `steps` based on `stitch_length` as       \
                `stitch_length` is too long compared to `radius`. A minimum \
                no. `steps` of 4 is chosen instead. To disable this either  \
                provide `steps`, decrease `stitch_length` or increase the   \
                circle `radius`"
            )
        else:
            extent = math.radians(extent * self._degreesPerAU)
            steps = self._n_sides_from_side_length(
                stitch_length, radius, extent
            )
    ...
    def circle(self, radius, extent=None, steps=None):
        """Draw a circle or arc, for more info see the official :py:func:
        `turtle.circle` documentation.
        ...
        steps: float
            The circle is approximated as a sequence of ``steps`` line
            segments. If the ``steps`` are not given, then the optimal number
            of line segments for the current stitch length is selected.
        """
        if radius == 0:  # TODO: Maybe use a lower tolerance
            warn(
                "Drawing a circle with radius is 0 is not possible and may \
                lead to many stitches in the same spot"
            )
        if math.isinf(radius) or math.isnan(radius):
            raise ValueError(
                f"``radius`` cannot be nan or inf, it is {radius}"
            )
        ...
        if (
            steps is None
            and self._stitch_group_stack  # Stitch group stack is not empty
            and hasattr(
                self._stitch_group_stack[-1], "stitch_length"
            )  # length is specified in topmost stitch group
        ):
            stitch_length = self._stitch_group_stack[-1].stitch_length
            steps = self._steps_from_stitch_length(
                stitch_length, abs(radius), extent
            )
        elif steps is None:
            steps = 20
    ...
    def start_running_stitch(self, stitch_length):
        ...
        ``running_stitch``-context instead).

        With a running stitch, we get stitches with a constant distance
        between each stitch.

        One step is equivalent to 0.1 mm, we recommend setting the minimum
        length between each stitch to 30 (3 mm).

        It is recommended to use the ``running_stitch``-context instead of the
        start-functions since they will automatically cleanup afterwards.
    ...
    def start_triple_stitch(self, stitch_length):
        ...
        ``triple_stitch``-context instead).

        Triple stitch is equivalent to running stitch, but the thread moves
        back and forth three times for each stitch.
    ...
    def start_jump_stitch(self):
        ...
        ``jump_stitch``-context instead).

        With a jump-stitch, trim the thread and move the needle without sewing
        more stitches.
        """
    ...
    def start_satin_stitch(self, width, center=True):
        ...
        self.set_stitch_type(
            stitches.SatinStitch(self.pos(), width, center=center)
        )
    ...
    def cleanup_stitch_type(self):
        ...
        if self._stitch_group_stack:
            # This handles nested context managers. We remove the top stitch
            # group from the stack and make a copy of it that we place back.
            # We do it this way so the starting position of the copy is where
            # the the turtle is right now.
    ...
    def use_stitch_group(self, stitch_group):
        ...
        yield
        if (
            self._stitch_group_stack[-1]._parent_stitch_group
            is not stitch_group
        ):
            raise RuntimeError(
                "Inconsistent state, likely caused by explicitly calling"
                + " `cleanup_stitch_type` within a stitch group context (e.g."
                + " within a `with turtle.running_stitch(20):` block)."
                + "\nYou should either set stitch groups with context managers"
                + " or with the `start_{stitch_type}` methods, not both."
            )
    ...
    def running_stitch(self, stitch_length):
        ...
        stitch_length : int
            Number of steps between each stitch.
        """
        return self.use_stitch_group(
            stitches.RunningStitch(self.pos(), stitch_length)
        )
    ...
    def triple_stitch(self, stitch_length):
        ...
        """
        return self.use_stitch_group(
            stitches.TripleStitch(self.pos(), stitch_length)
        )
    ...
    def jump_stitch(self, skip_intermediate_jumps=True):
        ...
        skip_intermediate_jumps : bool (optional, default=True)
            If True, then multiple jump commands will be collapsed into one
            jump command. This is useful in the cases where there may be
            multiple subsequent jumps with no stitches inbetween. Multiple
            subsequent jumps doesn't make sense but it can happen dependent on
            how you generate your patterns.
        """
        return self.use_stitch_group(
            stitches.JumpStitch(
                self.pos(), skip_intermediate_jumps=skip_intermediate_jumps
            )
        )
    ...
    def _position(self, other):
        """Goto a given position, see the :py:meth:`goto` documentation for
        more info."""
        if self._stitch_group_stack:
            self._stitch_group_stack[-1].add_location(other)
            if self.filling:
                self._fill_stitch_position_stack.append(other)
        self.x, self.y = other

    def save(self, filename):
        """Save the embroidery pattern as an embroidery or image file.

        Saves the embroiery pattern to file. Supports standard embroidery file
        formats, such as ``.dst``, ``.jef`` and ``.pes``, and utility formats
        such as ``.png``, ``.svg`` and ``.txt``. For a full list of supported
        file formats, see the `pyembroidery documentation
        <https://github.com/EmbroidePy/pyembroidery#file-io>`_.
        ...
        if not USE_SPHINX_GALLERY:
            ...
            self._gallery_patterns.append(
                (filename, self.pattern.to_pyembroidery())
            )

    def home(self):
        """Move the needle home (position (0, 0)), for more info see the
        official :py:func:`turtle.home` documentation"""
        self.goto(0, 0)
        self.angle = 0

    def visualise(
        self, turtle=None, width=800, height=800, scale=1, done=True, bye=True
    ):
        """Use the builtin ``turtle`` library to visualise this turtle's
        embroidery pattern.
        ...
        turtle : turtle.Turtle (optional)
            Python turtle object to use for drawing. If not specified, then
            the default turtle is used.
        width : int
            Canvas width
        height : int
            Canvas height
    ...
    def begin_fill(self, mode):
        """Begin filling an area..

        After begin_fill is called, the turtle will track the stitches made
        until end_fill is called, afterwhich the polygon formed by the
        stitches will be filled.

        The current implementation of fill is limited to straight lines
        between points of the polygon. This works well for "straight" stitches
        like running stitch and satin stitch, but will not fill in the spaces
        formed in stitches such as zigzag stitch.
    ...
    def end_fill(self):
        ...
        if (
            abs(
                self._fill_stitch_position_stack[0]
                - self._fill_stitch_position_stack[-1]
            )
            > 1
        ):
            self._fill_stitch_position_stack.append(
                self._fill_stitch_position_stack[0]
            )
